...
| B25 | files/texts received over Nearby Share never appear in chat; transfer completes but DB has zero rows for the session | `NearbyReceiverConnectionHandler.receiveTransfer` — only writes file to disk (fileManager) and emits receiveFlow; never calls `messageRepository.insertMessage`/`insertFileTransfer`; handler had no `MessageRepository` dependency at all | live-found macOS | `testNearbyShareReceivePersistsMessagesToRepository` | ✅ | ✅ | high/high — FIX: injected `MessageRepository` into `NearbyReceiverConnectionHandler` via `NearbyReceiverConnectionHandlerFactory` (+ DI at `CommunicationComponent.kt:210`). In `receiveTransfer`: pre-insert `insertFileTransfer(IN_PROGRESS)` per incoming file payload; on file chunk completion call `updateFileTransferFilePath`, `updateFileTransferStatus(COMPLETED)`, `insertMessage(isSender=false, FILE)`; on text payload completion call `insertMessage(isSender=false, TEXT)` — exact mirror of `FileMessageHandler.beginReceive` + `TextMessageHandler.handleIncoming` on the Klardrop path. Only persists on success; reject/failure paths leave no row (consistent with existing handlers). 312 suite tests green; `:klardrop-common:compileAndroidMain` + `:klardrop-common:compileKotlinMacosArm64` + `:klardrop-common:compileKotlinIosArm64` clean. |
| B26 | iOS/macOS chat list always empty — sent and received messages never render on Apple platforms | `iosApp/Observable/ChatModel.swift` line 29; `iosApp/Views/Chat/MessageRowView.swift` lines 33, 77, 170, 364; `iosApp/Views/Chat/DeviceChatScreen.swift` lines 72, 405 — all cast/type `Messages` (SQLDelight row) while the KMP VM emits `StateFlow<List<ChatMessage>>` since B22 changed the repo return type; every `as? [Messages]` cast fails silently, chat is perpetually empty | static (introduced by c7131ae SwiftUI migration + B22 repo type change) | — | ✅ | ✅ | high/high — FIX (B26, feature/reliability-hardening): changed all cast sites in `ChatModel.swift` from `as? [Messages]` / `as? Messages` to `as? [ChatMessage]` / `as? ChatMessage`; updated all struct field types in `MessageRowView.swift` (`MessageRowView`, `TextMessageBubble`, `FileMessageBubble`, `UnknownMessageBubble`) from `Messages` to `ChatMessage`; updated `DeviceChatScreen.swift` `sortedMessages: [ChatMessage]` and `MessageListView.messages: [ChatMessage]`; migrated field accesses from SQLDelight snake_case (`is_sender != 0`, `message_type`, `file_transfer_id`) to ChatMessage camelCase (`isSender`, `messageType`, `fileTransferId`). KlardropMac compiles clean. |
